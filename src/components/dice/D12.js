...
function makeFaceLabel(center, normal, number, offsetX = 0.175, offsetY = 0.2, scale = 0.55) {
  ...
  const offset = right.clone().multiplyScalar(offsetX).add(up.clone().multiplyScalar(offsetY));
...
function D12Mesh({
  ...
  React.useEffect(() => {
    ...
    vel.current.set(rollDirection[0] * 3, rollDirection[1] * 4, rollDirection[2] * 3);
  ...
  useFrame((_, delta) => {
    ...
    if (rolling) {
      ...
      if (pos.current.y < FLOOR_Y) {
        ...
        if (!bounced.current) {
          if (bounce) vel.current.y *= -BOUNCE_DAMPING;
...
export default function D12({ onRollComplete, bounce = true }) {
  ...
  const initialQuaternion = useMemo(() => {
    ...
    return new THREE.Quaternion().setFromUnitVectors(faceData[faceIndex].normal, forward);
  }, [faceData]);
  ...
  const rollDice = () => {
    ...
    // (keeping your existing target quaternion logic)
