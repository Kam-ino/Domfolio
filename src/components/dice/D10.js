...
function makeFaceLabel(center, normal, number, apexDir) {
  ...
  plane.position.copy(center).add(faceNormal.clone().multiplyScalar(0.02)).add(leftShift);
  plane.quaternion.copy(q);
  group.add(plane);

  numPlane.position.copy(center).add(faceNormal.clone().multiplyScalar(0.025)).add(leftShift);
...
export default function D10({ onRollComplete, bounce = true }) {
  ...
  const numberMeshes = useMemo(() => {
    return faceData.map((f, i) => makeFaceLabel(f.center, f.normal, i + 1, f.topDir));
  }, [faceData]);
